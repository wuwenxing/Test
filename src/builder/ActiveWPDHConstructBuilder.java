 ...
 *
 */
public class ActiveWPDHConstructBuilder implements ActiveBuilder {
	 ...
	 * @param active
	 */
	public ActiveWPDHConstructBuilder(Active active) {
		super();
		this.active = active;
	}
	...
	public Active createActiveBaseInfo(ActiveDTO param) {
		System.out.println("物品兑换型-建造活动基本信息");
		active.setType(ActTypeEnum.WPDH.getLabelKey());
		return active;
	}

	@Override
	public Active createActiveCondition(ActiveDTO param) {
		System.out.println("物品兑换型-建造活动参与条件");
		return active;
	}

	@Override
	public Active createActiveTask(ActiveDTO param) {
		System.out.println("物品兑换型-建造活动任务与奖励物品");
		return active;
	}
